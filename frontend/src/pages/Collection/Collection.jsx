...
const Collection = () => {
  const { products, searchTerm } = useContext(ShopContext); // Access products from context
  const { category } = useParams(); // Get category from URL

  // Filter products by category
  const filterProducts = products?.filter((product) =>
    product.category?.toLowerCase() === category?.toLowerCase() && product.name.toLowerCase().includes(searchTerm.toLowerCase())
  ) || [];
  ...
  // Select banner image
  const bannerImage = bannerImages[category?.toLowerCase()];
