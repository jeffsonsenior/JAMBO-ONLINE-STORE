  /**
    * Product data for the e-commerce site
    *  Each product has a unique ID, name, description, price, image, category, sizes, date, and bestseller   status
    * The image field contains an array of image URLs for the product
    * The sizes field contains an array of available sizes for the product
    * The date field represents the timestamp when the product was added
    * The bestseller field indicates whether the product is a bestseller or not
    * The products are sorted by category and bestseller status
    * The products are displayed in the order they appear in the array
    * The product images are stored in the assets folder
    * The product images are named p_img1, p_img2, p_img3, etc.
    * The product images are in the .jpg format
    * The product images are displayed in the order they appear in the array
    **/
import p_img1 from './women-top.jpg'
import p_img2_1 from './kid1.jpg'
import p_img2_2 from './kid1.1.jpg'
import p_img2_3 from './kid1.2.jpg'
import p_img2_4 from './kid1.3.jpg'
import p_img3 from './kid2.jpg'
import p_img4 from './men-t-2.jpg'
import p_img5 from './w-top2.jpg'
import p_img6 from './kid3.jpg'
import p_img7 from './men-t-3.jpg'
import p_img8 from './men-t-4.jpg'
import p_img9 from './kid4.jpg'
import p_img10 from './men-t-5.jpg'
import p_img11 from './men-t-6.jpg'
import p_img12 from './men-t-7.jpg'
import p_img13 from './w-top3.jpg'
import p_img14 from './kid5.jpg'
import p_img15 from './men-t-8.jpg'
import p_img16 from './kid6.jpg'
import p_img17 from './men-t-9.jpg'
import p_img18 from './kid7.jpg'
import p_img19 from './kid8.jpg'
import p_img20 from './w-top4.jpg'
import p_img21 from './w-top5.jpg'
import p_img22 from './w-top6.jpg'
import p_img23 from './kid11.jpg'
import p_img24 from './kid9.jpg'
import p_img25 from './kid10.jpg'
import p_img26 from './w-top7.jpg'
import p_img27 from './kid1.1.jpg'
import p_img28 from './men-t-10.jpg'
import p_img29 from './w-top8.jpg'




import razorpay_logo from './razorpay_logo.png'
import stripe_logo from './stripe_logo.png'
import bin_logo from './bin_logo.png'
import bunner_logo from './bunner_logo.png'
import mpesa_logo from './mpesa_logo.png'
import cart_logo from './cart_logo.png'
import profile_logo from './profile_logo.png'
import jambo_logo from './jambo_logo.png'

export const assets = {
    razorpay_logo,
    stripe_logo,
    bunner_logo,
    bin_logo,
    cart_logo,
    mpesa_logo,
    profile_logo,
    jambo_logo,
}
export const Products = [
    {
        _id: "a",
        name: "beatiful women dress",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1000,
        image: [p_img1],
        category: "Women",
        sizes: ["S", "M", "L"],
        date: 1716634345448,
        bestseller: true
    },
    {
        _id: "ab",
        name: "Kids Casual Fashion pair, trouser and Top",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 2000,
        image: [p_img2_1, p_img2_2, p_img2_3, p_img2_4],
        category: "Kids",
        sizes: ["M", "L", "XL"],
        date: 1716621345448,
        bestseller: true
    },
    {
        _id: "ac",
        name: "Kids Fashion bag",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 2200,
        image: [p_img3],
        category: "Kids",
        sizes: ["S", "L", "XL"],
        date: 1716234545448,
        bestseller: true
    },
    {
        _id: "ad",
        name: "Men Casaul Slim Fit Polo Shirt",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1100,
        image: [p_img4],
        category: "Men",
        sizes: ["S", "M", "XXL"],
        date: 1716621345448,
        bestseller: true
    },
    {
        _id: "ae",
        name: "Sexy Women Basic Casual Fitting black dress", 
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1300,
        image: [p_img5],
        category: "Women",
        sizes: ["M", "L", "XL"],
        date: 1716622345448,
        bestseller: true
    },
    {
        _id: "af",
        name: "Kids Fashion casual wear",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1400,
        image: [p_img6],
        category: "Kids",
        sizes: ["S", "L", "XL"],
        date: 1716623423448,
        bestseller: true
    },
    {
        _id: "ag",
        name: "Men polo-fit shirt",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1900,
        image: [p_img7],
        category: "Men",
        sizes: ["S", "L", "XL"],
        date: 1716621542448,
        bestseller: false
    },
    {
        _id: "ah",
        name: "Men Casaul Slim Fit Polo Shirt",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1440,
        image: [p_img8],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716622345448,
        bestseller: false
    },
    {
        _id: "ai",
        name: "Kids Fashion designers dress",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1030,
        image: [p_img9],
        category: "Kids",
        sizes: ["M", "L", "XL"],
        date: 1716621235448,
        bestseller: false
    },
    {
        _id: "aj",
        name: "Men Red polo slim fit shirt",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1800,
        image: [p_img10],
        category: "Men",
        sizes: ["S", "L", "XL"],
        date: 1716622235448,
        bestseller: false
    },
    {
        _id: "ak",
        name: "Men trouser",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1300,
        image: [p_img11],
        category: "Men",
        sizes: ["S", "M", "L"],
        date: 1716623345448,
        bestseller: false
    },
    {
        _id: "al",
        name: "Men Casaul fit khaki pants",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 2500,
        image: [p_img12],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716624445448,
        bestseller: false
    },
    {
        _id: "am",
        name: "Women elegant blue jeans trouser",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1030,
        image: [p_img13],
        category: "Women",
        sizes: ["S", "M", "L", "XL"],
        date: 1716625545448,
        bestseller: false
    },
    {
        _id: "an",
        name: "Girls Casaul Slim Fit dress",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 960,
        image: [p_img14],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716626645448,
        bestseller: false
    },
    {
        _id: "ao",
        name: "Men Tapered Fit Trousers",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1240,
        image: [p_img15],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716627745448,
        bestseller: false
    },
    {
        _id: "ap",
        name: "Kids Fashion Slim Fit Casual vest",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1570,
        image: [p_img16],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716628845448,
        bestseller: false
    },
    {
        _id: "aq",
        name: "Men Tapered Fit Flat-Front Trousers",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1150,
        image: [p_img17],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716629945448,
        bestseller: false
    },
    {
        _id: "ar",
        name: "Boy Casaul Slim Fit Polo Shirt",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 1680,
        image: [p_img18],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716631045448,
        bestseller: false
    },
    {
        _id: "as",
        name: "Boy Casaul Slim Fit Polo Shirt",
        description: "  Lorem ipsum dolor sit amet consectetur adipisicing elit. In veniam deleniti iure. Natus ab quisquam delectus voluptatem ipsum laboriosam laudantium quia vero rem earum, accusamus officia cumque vel architecto iste.",
        price: 2160,
        image: [p_img19],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716632145448,
        bestseller: false
    },
    {
        _id: "at",
        name: " Woman casual wear, Palazzo Pants",
        description: "Embrace effortless elegance with these stylish palazzo pants, designed for ultimate comfort and a flattering fit. Perfect for casual outings or relaxed weekends, these pants feature a flowy design that pairs easily with any top for a chic, laid-back look.",
        price: 1190,
        image: [p_img20],
        category: "Women",
        sizes: ["S", "M", "L", "XL"],
        date: 1716633245448,
        bestseller: false
    },
    {
        _id: "au",
        name: "Women trouser",
        description: " Stay comfortable and fashionable with these versatile women's trousers. Crafted for all-day wear, they offer a sleek, flattering silhouette and are perfect for both work and casual settings. Available in a variety of sizes to ensure the perfect fit.",
        price: 1270,
        image: [p_img21],
        category: "Women",
        subCategory: "Winterwear",
        sizes: ["S", "M", "L", "XL"],
        date: 1716634345448,
        bestseller: false
    },
    {
        _id: "av",
        name: "Women fitting dress",
        description: "  This women's fitted dress is designed to enhance your natural shape with a tailored, form-fitting cut. Ideal for a night out or special occasions, it combines elegance with modern style, making it a must-have addition to your wardrobe.",
        price: 2030,
        image: [p_img22],
        category: "Women",
        sizes: ["S", "M", "L", "XL"],
        date: 1716635445448,
        bestseller: false
    },
    {
        _id: "aw",
        name: "Boy Casaul Slim Fit Polo Shirt",
        description: "  Keep your little one stylish with this slim fit polo shirt. Designed for comfort and style, it features a classic polo collar and a modern slim cut that is perfect for casual wear or more polished looks.",
        price: 1840,
        image: [p_img23],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716636545448,
        bestseller: false
    },
    {
        _id: "ax",
        name: "Boy Casaul worm sweater and mavin",
        description: "  This cozy warm sweater and mavin set is perfect for colder weather. Designed for comfort and warmth, it combines style with practicality, making it an ideal choice for your child is winter wardrobe.",
        price: 1010,
        image: [p_img24],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716637645448,
        bestseller: false
    },
    {
        _id: "ay",
        name: "Kids Fashion Slim Fit Casual",
        description: "  Elevate your child is wardrobe with this slim-fit casual outfit, perfect for everyday wear. Designed with a modern cut, it ensures both comfort and style, making it easy for your little one to stay fashionable while on the go.",
        price: 1290,
        image: [p_img25],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716638745448,
        bestseller: false
    },
    {
        _id: "az",
        name: "Woman casual shirt",
        description: "  This casual shirt for women combines comfort with style, making it perfect for everyday wear. With its simple design and breathable fabric, it offers a relaxed fit that pairs effortlessly with jeans, skirts, or shorts.",
        price: 1320,
        image: [p_img26],
        category: "Women",
        subCategory: "Winterwear",
        sizes: ["S", "M", "L", "XL"],
        date: 1716639845448,
        bestseller: false
    },
    {
        _id: "ba",
        name: "Kids Fashion",
        description: "  A fun and fashionable outfit for kids, designed to keep them looking trendy and feeling comfortable. Perfect for any occasion, this piece is made with soft materials that provide all-day comfort without compromising on style.",
        price: 4200,
        image: [p_img27],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716640945448,
        bestseller: false
    },
    {
        _id: "bb",
        name: "Men Slim Fit khaki pants",
        description: "  These men's slim-fit khaki pants offer a sleek, modern look with a comfortable fit. Perfect for both casual and semi-formal occasions, they feature durable fabric and a versatile design that pairs well with any shirt or jackets, everyday cool.",
        price: 2230,
        image: [p_img28],
        category: "Men",
        subCategory: "Winterwear",
        sizes: ["S", "M", "L", "XL"],
        date: 1716642045448,
        bestseller: false
    },
    {
        _id: "bc",
        name: "Women Basic Casual Fitting Top",
        description: " Elevate your everyday wardrobe with our Women's Basic Casual Fitted Top, crafted from a soft, breathable cotton-spandex blend for a flattering fit that moves with you. Featuring a timeless design with a round neckline and versatile fit,perfect for casual outings, workdays, or nights out. Durable and easy to care for, this top retains its shape and color after multiple washes and comes in a variety of vibrant and neutral tones. Pair it effortlessly with jeans, skirts, or shorts for a chic, everyday look.",
        price: 2410,
        image: [p_img29],
        category: "Women",
        sizes: ["S", "M", "L", "XL"],
        date: 1716643145448,
        bestseller: false
    },
]

/**
export const Products = [  
    {
        _id: "a",
        name: "Beautiful Women's Dress",
        description: "Elegant and stylish, this dress is perfect for any occasion. Crafted with premium materials, it offers a comfortable fit and a timeless design.",
        price: 1000,
        image: [p_img1],
        category: "Women",
        sizes: ["S", "M", "L"],
        date: 1716634345448,
        bestseller: true
    },
    {
        _id: "ab",
        name: "Kids Casual Fashion Pair - Trouser and Top",
        description: "This trendy trouser and top set combines comfort with fashion, making it ideal for everyday wear. Designed with vibrant colors and durable fabric for active kids.",
        price: 2000,
        image: [p_img2_1, p_img2_2, p_img2_3, p_img2_4],
        category: "Kids",
        sizes: ["M", "L", "XL"],
        date: 1716621345448,
        bestseller: true
    },
    {
        _id: "ac",
        name: "Kids Fashion Bag",
        description: "Stylish and functional, this bag is perfect for kids to carry their essentials. Lightweight and durable with a playful design.",
        price: 2200,
        image: [p_img3],
        category: "Kids",
        sizes: ["S", "L", "XL"],
        date: 1716234545448,
        bestseller: true
    },
    {
        _id: "ad",
        name: "Men's Casual Slim Fit Polo Shirt",
        description: "A versatile slim-fit polo shirt that offers a sleek and modern look. Made from breathable fabric for all-day comfort.",
        price: 1100,
        image: [p_img4],
        category: "Men",
        sizes: ["S", "M", "XXL"],
        date: 1716621345448,
        bestseller: true
    },
    {
        _id: "ae",
        name: "Womens Basic Casual Fitting Black Dress",
        description: "A classic black dress with a flattering fit. Perfect for casual outings or dressing up for a night out.",
        price: 1300,
        image: [p_img5],
        category: "Women",
        sizes: ["M", "L", "XL"],
        date: 1716622345448,
        bestseller: true
    },
    {
        _id: "af",
        name: "Kids Fashion Casual Wear",
        description: "This casual outfit combines style and comfort, making it a great choice for kids' everyday adventures.",
        price: 1400,
        image: [p_img6],
        category: "Kids",
        sizes: ["S", "L", "XL"],
        date: 1716623423448,
        bestseller: true
    },
    {
        _id: "ag",
        name: "Men's Polo-Fit Shirt",
        description: "A refined polo-fit shirt designed for casual and semi-formal occasions. Made from high-quality fabric for durability.",
        price: 1900,
        image: [p_img7],
        category: "Men",
        sizes: ["S", "L", "XL"],
        date: 1716621542448,
        bestseller: false
    },
    {
        _id: "ah",
        name: "Men's Casual Slim Fit Polo Shirt",
        description: "A stylish slim-fit polo shirt perfect for any casual occasion. Designed for a polished yet relaxed look.",
        price: 1440,
        image: [p_img8],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716622345448,
        bestseller: false
    },
    {
        _id: "ai",
        name: "Kids Fashion Designer's Dress",
        description: "An adorable designer dress that blends style and comfort for your little one. Perfect for special occasions or casual wear.",
        price: 1030,
        image: [p_img9],
        category: "Kids",
        sizes: ["M", "L", "XL"],
        date: 1716621235448,
        bestseller: false
    },
    {
        _id: "aj",
        name: "Men's Red Polo Slim Fit Shirt",
        description: "Bold and vibrant, this red slim-fit polo shirt adds a pop of color to your wardrobe. Comfortable and versatile.",
        price: 1800,
        image: [p_img10],
        category: "Men",
        sizes: ["S", "L", "XL"],
        date: 1716622235448,
        bestseller: false
    },
    {
        _id: "ak",
        name: "Men's Trousers",
        description: "Classic trousers that combine comfort and style. Perfect for both casual and semi-formal occasions.",
        price: 1300,
        image: [p_img11],
        category: "Men",
        sizes: ["S", "M", "L"],
        date: 1716623345448,
        bestseller: false
    },
    {
        _id: "al",
        name: "Men's Casual Fit Khaki Pants",
        description: "Versatile khaki pants designed for a comfortable and stylish fit. Ideal for both casual and work settings.",
        price: 2500,
        image: [p_img12],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716624445448,
        bestseller: false
    },
    {
        _id: "am",
        name: "Women's Elegant Blue Jeans Trouser",
        description: "Chic and durable, these blue jeans are a wardrobe staple for any modern woman. Perfect for everyday wear.",
        price: 1030,
        image: [p_img13],
        category: "Women",
        sizes: ["S", "M", "L", "XL"],
        date: 1716625545448,
        bestseller: false
    },
    {
        _id: "an",
        name: "Girls' Casual Slim Fit Dress",
        description: "A charming slim-fit dress designed for young girls. Perfect for casual outings or family gatherings.",
        price: 960,
        image: [p_img14],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716626645448,
        bestseller: false
    },
    {
        _id: "ao",
        name: "Men's Tapered Fit Trousers",
        description: "Stylish tapered-fit trousers that deliver a sleek and modern look. Great for work or casual settings.",
        price: 1240,
        image: [p_img15],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716627745448,
        bestseller: false
    },
    {
        _id: "ap",
        name: "Kids' Fashion Slim Fit Casual Vest",
        description: "A trendy casual vest for kids, offering both style and comfort. Perfect for layering or wearing on its own.",
        price: 1570,
        image: [p_img16],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716628845448,
        bestseller: false
    },
    {
        _id: "aq",
        name: "Men's Tapered Fit Flat-Front Trousers",
        description: "Classic flat-front trousers with a tapered fit. A versatile addition to any wardrobe.",
        price: 1150,
        image: [p_img17],
        category: "Men",
        sizes: ["S", "M", "L", "XL"],
        date: 1716629945448,
        bestseller: false
    },
    {
        _id: "ar",
        name: "Boys' Casual Slim Fit Polo Shirt",
        description: "A stylish polo shirt for boys, combining comfort and a polished look. Great for everyday wear.",
        price: 1680,
        image: [p_img18],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716631045448,
        bestseller: false
    },
    {
        _id: "as",
        name: "Boys' Casual Slim Fit Polo Shirt",
        description: "Comfortable and trendy, this slim-fit polo shirt is perfect for active boys. Ideal for any casual occasion.",
        price: 2160,
        image: [p_img19],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716632145448,
        bestseller: false
    },
    {
        _id: "at",
        name: "Women's Casual Wear - Palazzo Pants",
        description: "Relaxed and chic, these palazzo pants are perfect for casual outings or lounging.",
        price: 1570,
        image: [p_img21],
        category: "Women",
        subCategory: "Winterwear",
        sizes: ["S", "M", "L", "XL"],
        date: 1716634345448,
        bestseller: false
    {
        _id: "au",
        name: "Women trouser",
        description: "Stay comfortable and fashionable with these versatile women's trousers. Crafted for all-day wear, they offer a sleek, flattering silhouette and are perfect for both work and casual settings. Available in a variety of sizes to ensure the perfect fit.",
        price: 1270,
        image: [p_img21],
        category: "Women",
        subCategory: "Winterwear",
        sizes: ["S", "M", "L", "XL"],
        date: 1716634345448,
        bestseller: false
    },
    {
        _id: "av",
        name: "Women Fitting Dress",
        description: "This women's fitted dress is designed to enhance your natural shape with a tailored, form-fitting cut. Ideal for a night out or special occasions, it combines elegance with modern style, making it a must-have addition to your wardrobe.",
        price: 2030,
        image: [p_img22],
        category: "Women",
        sizes: ["S", "M", "L", "XL"],
        date: 1716635445448,
        bestseller: false
    },
    {
        _id: "aw",
        name: "Boy Casual Slim Fit Polo Shirt",
        description: "Keep your little one stylish with this slim fit polo shirt. Designed for comfort and style, it features a classic polo collar and a modern slim cut that is perfect for casual wear or more polished looks.",
        price: 1840,
        image: [p_img23],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716636545448,
        bestseller: false
    },
    {
        _id: "ax",
        name: "Boy Casual Warm Sweater and Mavin",
        description: "This cozy warm sweater and mavin set is perfect for colder weather. Designed for comfort and warmth, it combines style with practicality, making it an ideal choice for your child's winter wardrobe.",
        price: 1010,
        image: [p_img24],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716637645448,
        bestseller: false
    },
    {
        _id: "ay",
        name: "Kids Fashion Slim Fit Casual",
        description: "Elevate your child's wardrobe with this slim-fit casual outfit, perfect for everyday wear. Designed with a modern cut, it ensures both comfort and style, making it easy for your little one to stay fashionable while on the go.",
        price: 1290,
        image: [p_img25],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716638745448,
        bestseller: false
    },
    {
        _id: "az",
        name: "Women Casual Shirt",
        description: "This casual shirt for women combines comfort with style, making it perfect for everyday wear. With its simple design and breathable fabric, it offers a relaxed fit that pairs effortlessly with jeans, skirts, or shorts.",
        price: 1320,
        image: [p_img26],
        category: "Women",
        subCategory: "Winterwear",
        sizes: ["S", "M", "L", "XL"],
        date: 1716639845448,
        bestseller: false
    },
    {
        _id: "ba",
        name: "Kids Fashion",
        description: "A fun and fashionable outfit for kids, designed to keep them looking trendy and feeling comfortable. Perfect for any occasion, this piece is made with soft materials that provide all-day comfort without compromising on style.",
        price: 4200,
        image: [p_img27],
        category: "Kids",
        sizes: ["S", "M", "L", "XL"],
        date: 1716640945448,
        bestseller: false
    },
    {
        _id: "bb",
        name: "Men Slim Fit Khaki Pants",
        description: "These men's slim-fit khaki pants offer a sleek, modern look with a comfortable fit. Perfect for both casual and semi-formal occasions, they feature durable fabric and a versatile design that pairs well with any shirt or jacket for an everyday cool look.",
        price: 2230,
        image: [p_img28],
        category: "Men",
        subCategory: "Winterwear",
        sizes: ["S", "M", "L", "XL"],
        date: 1716642045448,
        bestseller: false
    },
    {
        _id: "bc",
        name: "Women Basic Casual Fitting Top",
        description: "Elevate your everyday wardrobe with our Women's Basic Casual Fitted Top, crafted from a soft, breathable cotton-spandex blend for a flattering fit that moves with you. Featuring a timeless design with a round neckline and versatile fit, perfect for casual outings, workdays, or nights out. Durable and easy to care for, this top retains its shape and color after multiple washes and comes in a variety of vibrant and neutral tones. Pair it effortlessly with jeans, skirts, or shorts for a chic, everyday look.",
        price: 2410,
        image: [p_img29],
        category: "Women",
        sizes: ["S", "M", "L", "XL"],
        date: 1716643145448,
        bestseller: false
    }
]

**/